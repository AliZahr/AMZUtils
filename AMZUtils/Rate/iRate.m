...
NSString * const iRateiOSAppStoreURLFormat = @"itms-apps://itunes.apple.com/app/id%lu";
...
@implementation iRate
{
    ...
    UIAlertAction* rateButton;
    UIAlertAction* remindButton;
...
- (iRate *)init
{
    if ((self = [super init]))
    {
        ...
        rateButton = [UIAlertAction
                      actionWithTitle:rateButtonLabel
                      style:UIAlertActionStyleDefault
                      handler:^(UIAlertAction * action) {
            self.ratedThisVersion = YES;
            [self openRatingsPageInAppStore];
        }];
        remindButton = [UIAlertAction
                        actionWithTitle:remindButtonLabel
                        style:UIAlertActionStyleDefault
                        handler:^(UIAlertAction * action) {
...
- (NSURL *)ratingsURL
{
    ...
    return [NSURL URLWithString:[NSString stringWithFormat:iRateiOSAppStoreURLFormat, (unsigned long)appStoreID]];
...
- (void)promptForRating
{
    ...
    UIAlertController * alert = [UIAlertController
                                 alertControllerWithTitle:messageTitle
...
- (void)promptIfNetworkAvailable
{
    ...
    NSMutableURLRequest *urlRequest = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://apple.com"]];
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:urlRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
    {
      ...
      NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                if ([(NSObject *)strongSelf.delegate respondsToSelector:@selector(iRateShouldShouldPromptForRating)])
                {
                    if (![strongSelf.delegate iRateShouldShouldPromptForRating])
                    {
                        return;
                    }
                }
                ...
                //prompt user
                [strongSelf promptForRating];
            }
            else
            {
                //could not connect
                if ([(NSObject *)strongSelf.delegate respondsToSelector:@selector(iRateCouldNotConnectToAppStore:)])
                {
                    [strongSelf.delegate iRateCouldNotConnectToAppStore:error];
                }
            }
        });
    }];
